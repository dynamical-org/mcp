...
dynamical.org's own public STAC catalog and status feed, so there's
nothing to inject. v2's planned GitHub OAuth gating would add a
...
Setup:
    ...
    One-time (once a subdomain is chosen, e.g. mcp.dynamical.org): add
    `custom_domains=[...]` to the `@modal.asgi_app()` call below, then
    create the CNAME record per the Modal dashboard's instructions
    (DNS-only, matching status.dynamical.org's setup).
...
image = (
    ...
    .pip_install("mcp[cli]>=1.28.0", "httpx>=0.27")
